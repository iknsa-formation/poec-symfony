...
$fonction = $_POST ['fonction'];
    ...
    if ($var > 0){
      echo "<div class= 'alert alert-alert' role= 'alert'> <h3>>>>>>>>>>>>>>>> existe déjà <<<<<<<<<<<<<<<</h3> </div>";
    } else {
    ...
    L'utilisateur <h3>>>>>>>>>>>>>>>>  $prenom $nom a été ajouté en base! <<<<<<<<<<<<<<<<<</h3></div>";
    } else {
...
<!--
    ...
    // try
    // {
    // 	$bdd = new PDO('mysql:host=localhost;dbname=poec-symfony;charset=utf8', 'root', '');
    // }
    // catch(Exception $e)
    // {
    ...
    // }
    //
    // $reponse = $bdd->query('SELECT lastname FROM user');
    //
